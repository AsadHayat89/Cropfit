...
class DairyListScreen extends StatefulWidget {
  const DairyListScreen({Key? key}) : super(key: key);
...
class _DairyListScreenState extends State<DairyListScreen> {
  ...
  Widget build(BuildContext context) {
    Stream<QuerySnapshot> cropsStream = db.collection("crops").where('type', isEqualTo: 'Dairy').where('farmerId', isEqualTo: UserPreferences.user.email).snapshots();
    ...
    return Scaffold(
      ...
      body: ListView(
        children: [
          SizedBox(height: 30,),
          Center(child: Text('\My Dairy Catalog',style: TextStyle(fontSize: 25,fontWeight: FontWeight.bold),),),
          SizedBox(height: 30,),
          StreamBuilder(
              stream: cropsStream,
            ...
            child:    Row(
              ...
              children: [
                Column(
                  ...
                  children: [
                    Text('Registered Your Dairy', style: TextStyle(fontWeight: FontWeight.bold,fontSize: 20),),
                  ],),
