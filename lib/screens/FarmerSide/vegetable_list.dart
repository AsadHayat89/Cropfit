...
class _VegetableListScreenState extends State<VegetableListScreen> {
  ...
  Widget build(BuildContext context) {
    Stream<QuerySnapshot> cropsStream = db.collection("crops").where('type', isEqualTo: 'Vegetable').where('farmerId', isEqualTo: UserPreferences.user.email).snapshots();
    ...
    return Scaffold(
      ...
      body: ListView(
        children: [
          SizedBox(height: 30,),
          Center(child: Text('My Vegetables Catalog',style: TextStyle(fontSize: 25,fontWeight: FontWeight.bold),),),
          SizedBox(height: 30,),
          StreamBuilder(
            stream: cropsStream,
