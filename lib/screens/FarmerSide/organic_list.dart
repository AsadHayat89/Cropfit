...
class OrganicListScreen extends StatefulWidget {
  const OrganicListScreen({Key? key}) : super(key: key);

  @override
  State<OrganicListScreen> createState() => _OrganicListScreenState();
}

class _OrganicListScreenState extends State<OrganicListScreen> {
  @override
  Widget build(BuildContext context) {
    Stream<QuerySnapshot> cropsStream = db.collection("crops").where('type', isEqualTo: 'Organic').where('farmerId', isEqualTo: UserPreferences.user.email).snapshots();
    ...
    return Scaffold(
      ...
      body: ListView(
        children: [
          SizedBox(height: 30,),
          Center(child: Text('My Organic Catalog',style: TextStyle(fontSize: 25,fontWeight: FontWeight.bold),),),
          SizedBox(height: 30,),
          StreamBuilder(
              stream: cropsStream,
            ...
            child:    Row(
              ...
              children: [
                Column(
                  ...
                  children: [
                    Text('Registered Your Crops', style: TextStyle(fontWeight: FontWeight.bold,fontSize: 20),),
                  ],),
                ...
                Btn(onClicked: () {
                  db.collection("crops").doc(crop.id).delete().then((value) {
                    ...
                    ScaffoldMessenger.of(context)
                        .showSnackBar(SnackBar(content: Text('Crop deleted successfully!')));
                    /* MotionToast.success(
