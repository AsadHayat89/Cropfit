...
class FarmerEditProfile extends StatefulWidget {
  const FarmerEditProfile({Key? key}) : super(key: key);
...
class _FarmerEditProfileState extends State<FarmerEditProfile> {
  ...
  updateUser(User user) {
    ...
    db.collection("users").doc(user.email).update(user.toJson()).then((value) async {
      ...
      await storageProfilePics.child(UserPreferences.user.email).child('profile.jpg').putData(cropImage!).then((p0) async {
      ...
      await storageCoverPics.child(UserPreferences.user.email).child('cover.jpg').putData(cropCoverImage!).then((p0) async {
        await storageCoverPics.child(UserPreferences.user.email).child('cover.jpg').getDownloadURL().then((value) {
          user.coverUrl = value;
          db.collection("users").doc(user.email).update({"coverUrl": value}).then((value) {
            ...
            MotionToast.success(
              title: Text("User Updated! " + user.name.toString(), style: TextStyle(fontWeight: FontWeight.bold),
              ),
